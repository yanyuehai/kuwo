...
function ranking() {
    for (let index = 0; index < 5; index++) {
        fetch(replaceHost(topurl[index])).then(res => res.json()).then((data) => {
            bangTitle(data, index)
            bangList(data.data.musicList, index)
        })
    }
}
...
function singer(url, index) {
    ...
    $('.playlist').eq(index).html('')
    fetch(replaceHost(url)).then(res => res.json()).then((data) => {
        singerList(data.data.artistList, index)
    })
}


// 推荐歌手渲染函数
function singerList(data, index) {
    data.forEach((v, i) => {
        $(`
        <li>
